---
name: rn-editorial-collage-motion
...
```bash
COLLAGE_SKILL_DIR="/absolute/path/to/rn-editorial-collage-motion"
python3 "$COLLAGE_SKILL_DIR/scripts/validate_collage.py" \
...
```bash
python3 "$COLLAGE_SKILL_DIR/scripts/render_assembly.py" \
  --project /absolute/path/project \
  --plan /absolute/path/assembly-plan.json \
